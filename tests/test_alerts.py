...
class TestShowErrorAlert:
    ...
    def test_darwin_creates_alert(self):
        ...
        with patch.dict("sys.modules", {
            ...
            utils.alerts.show_error_alert("API-Key fehlt", "DEEPGRAM_API_KEY nicht gesetzt")
        
        # Verify NSAlert wurde konfiguriert
        mock_alert_instance.setMessageText_.assert_called_once_with("API-Key fehlt")
        mock_alert_instance.setInformativeText_.assert_called_once_with("DEEPGRAM_API_KEY nicht gesetzt")
    ...
    def test_import_error_falls_back_to_logging(self, caplog):
        ...
        mock_appkit.NSAlert.alloc.return_value.init.side_effect = Exception("Simulated error")
        ...
        with patch.dict("sys.modules", {
            ...
            assert "Alert konnte nicht angezeigt werden" in caplog.text or "Test" in caplog.text
